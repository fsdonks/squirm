...
def columns(tbl):
    width = len(tbl[0])
    height = len(tbl)
    return ([tbl[i][j] for i in xrange(height)] for j in xrange(width))
...
def cells(tbl):
   width = len(tbl[0])
   height = len(tbl)
   return ([((i,j), tbl[i][j]) for i in xrange(height)] for j in xrange(width))
...
def merge (dicts):
    res = {}
    for d in dicts:
        reduce(lambda acc, (k,v): assoc(acc,k,v),d.iteritems(),res)
    return res
#conjoin multiple items into a set.
def conj(s,xs):
    reduce(lambda acc,x: s.add(x), xs, s)
    return s
